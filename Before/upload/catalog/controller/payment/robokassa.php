...
class ControllerPaymentRobokassa extends Controller {
	...
	protected function index() {
		...
		if( $STORE_ID )
		{
			...
			if( $this->is_serialized($CONFIG['robokassa_desc']) )
			$CONFIG['robokassa_desc'] = unserialize($CONFIG['robokassa_desc']);
			
			if( isset($CONFIG['robokassa_desc'][$STORE_ID][ $this->config->get('config_language') ]) )
			$CONFIG['robokassa_desc'] = $CONFIG['robokassa_desc'][$STORE_ID][ $this->config->get('config_language') ];
			else
			$CONFIG['robokassa_desc'] = '';
		...
		$this->data['inv_desc'] = str_replace("{name}", $order_info['name'], $this->data['inv_desc']);
	...
	public function result() 
	{
		...
		if( $STORE_ID )
		{
			...
			$CONFIG['robokassa_premod_order_status_id'] = $this->config->get('robokassa_premod_order_status_id');
		...
		else
		{
			...
			$CONFIG['robokassa_premod_order_status_id'] = $this->config->get('robokassa_premod_order_status_id_store');
		...
		)
		{
			...
			elseif(  $CONFIG['robokassa_confirm_status']=='premod'  )
			{
				...
				$this->model_checkout_order->confirm($inv_id, $CONFIG['robokassa_premod_order_status_id'], $comment, false);
			}
