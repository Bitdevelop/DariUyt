...
$(document).ready(function(){
	...
	var select_elem = $('.ssss').nextAll();
	for (var i=0;i<select_elem.length;i++) {
		select_elem[i].remove();
	}
	// console.log ($.isEmptyObject(select_elem) == false);
	// while ($.isEmptyObject(select_elem) == false) {
		// console.log(select_elem);
		// select_elem = $(select_elem).next();
		// $(select_elem).prev().remove();
	// }
})
function getUrlVars() {
    var vars = {};
