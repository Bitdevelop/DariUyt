...
class ControllerModuleCSVImport extends Controller {
	...
	public function export() {
		...
		$sql1 = "SELECT product_id, NAME FROM ". DB_PREFIX. "product_description";
		$sql2 = "SELECT manufacturer_id, name FROM ". DB_PREFIX. "manufacturer";
		header('Content-Type: text/csv');
		header('Content-Disposition: attachment;filename=' . 'export.csv');
		$unset = array('date_available','upc','stock_status_id','ean','jan', 'isbn','mpn','location','image','manufacturer_id','shipping','points','tax_class_id','weight','weight_class_id','length','width','height','length_class_id','subtract','minimum','sort_order','status','date_added','date_modified','viewed');
		$fp = fopen('php://output', 'w');		
		$query = $this->db->query($sql);
		$query1 = $this->db->query($sql1);
		$query2 = $this->db->query($sql2);
		foreach ($query1->rows as $item){
			$items[$item['product_id']] = iconv('UTF8', 'CP1251', $item['NAME']);
		}		
		foreach ($query2->rows as $manuf){
			$manufacturers[$manuf['manufacturer_id']] = iconv('UTF8', 'CP1251', $manuf['name']);
		}
		$manufacturers['0'] = '';
		$manuftitle = 'manufacturer';
		$query->row =  array_slice($query->row, 0, 3, true) + array("name" => "") + array_slice($query->row, 3, count($query->row)-3, true);
		$query->row =  array_slice($query->row, 0, 2, true) + array($manuftitle => "") + array_slice($query->row, 2, count($query->row)-2, true);		
		foreach ($unset as $uset){
			unset($query->row[$uset]);
		}
		if($query->rows) {
			...
			foreach ($translation as $key =>$value){
				$trans[$key]=iconv('UTF8', 'CP1251', $value);
			}
			fputcsv($fp, $trans, ';', '"');
			foreach($query->rows as $row) {
				$row =  array_slice($row, 0, 3, true) + array("name" => $items[$row['product_id']]) + array_slice($row, 3, count($row)-3, true);
				$row =  array_slice($row, 0, 2, true) + array($manuftitle => $manufacturers[$row['manufacturer_id']]) + array_slice($row, 2, count($row)-2, true);
