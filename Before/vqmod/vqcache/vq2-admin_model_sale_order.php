...
class ModelSaleOrder extends Model {
	...
	public function getOrders($data = array()) {
			...
			$sql = "SELECT o.order_id, CONCAT(o.firstname, ' ', o.lastname) AS customer, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status, (SELECT os.status_bg_color FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status_bg_color, (SELECT os.status_txt_color FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status_txt_color, o.total, o.order_status_id, o.currency_code, o.currency_value, o.date_added, o.date_modified FROM `" . DB_PREFIX . "order` o";
...
// для chronopay
			/*$payment_code = array(
				'chronopay_payment_type_card',
				'chronopay_payment_type_yandexmoney',
				'chronopay_payment_type_webmoney',
				'cod'
			);*/
// для robokassa
			$payment_code = array(
				'robokassa', // QIWI Кошелек
				'robokassa1', // Яндекс.Деньги
				'robokassa2', // WMR
				'robokassa4', // RUR Банковская карта
				'cod' // оплата при доставке
			);

			if (((int)$data['order_status_id'] == 2) && (in_array($order_info['payment_code'], $payment_code))) {
				...
				//все можно перенсти в язык но мне влом, сорри))
				$template->data['customer'] = 'Добрый день, '.$order_info['firstname'].' '.$order_info['lastname'].'!';
				$template->data['order'] = 'Заказ №'.$order_id.'!';

				$template->data['telephoneBack'] = $this->config->get('config_telephone');
				$template->data['emailBack'] = 'info@dariyut.ru';

				$template->data['total'] = $order_info['total'];

				$template->data['text_footer'] ='Данное письмо было сформировано автоматически, отвечать на него не требуется.';
				...
				foreach ($order_product_query->rows as $product) {
					...
					$sku_info_produck = $this->db->query("SELECT sku FROM " . DB_PREFIX . "product WHERE  product_id = '" . (int)$product['product_id'] . "'");
					$sku = 0;
					if ($sku_info_produck->rows)
						$sku = $sku_info_produck->rows[0]["sku"];
					$template->data['products'][] = array(
						'name'     => $product['name'],
						'model'    => $product['model'],
						'sku'    => $sku,
				...
				// 	);
				// }
// echo $order_info['payment_code'];exit;
				if (in_array($order_info['payment_code'], $payment_code) && ($order_info['payment_code'] != 'cod')) {
					$template_link = 'sale/mail_chronopay.tpl';
					//ссылка на оплату через chronopay
					$template->data['link'] = $order_info['store_url'] . 'index.php?route=payment/robokassa/getRedirectURL&order_id=' . $order_id;
					$template->data['title_link'] = 'оплатить здесь (RoboKassa).';
				}

				if ($order_info['payment_code'] == 'cod') {
					$template_link = 'sale/mail_cod.tpl';
					$template->data['link1'] = $order_info['store_url'] . 'pay';
					$template->data['title_link1'] = 'оплата';
					$template->data['link2'] = $order_info['store_url'] . 'index.php?route=information/information&information_id=6';
					$template->data['title_link2'] = 'доставка';
				}
			...
			if (isset($template)) {
				$mail->setHtml($template->fetch($template_link));
				$mail->send();
			}
		}
	}
