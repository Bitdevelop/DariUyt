...
'description2'      => $query->row['description2'],
		...
		//				'stock_status'     => $stock_status,//[Замена]//
				...
				'diam'  => $query->row['diam'],
				'diam_class_id'  => $query->row['diam_class_id'],
...
public function getProducts($data = array()) {
	...
	if (!empty($data['sort'])){
		...
		if (isset($data['price_from'])){
					if ((int)$data['price_from']>0){
					$sql.=" AND p.price>=".$data['price_from']." ";
					}
					}
					if (isset($data['price_to'])){
					if ((int)$data['price_to']>0){
					$sql.=" AND p.price<=".$data['price_to']." ";
					}
					}
...
public function getModels($id,$path = NULL) {
	if ($id) {
		if ($path)
			$sql = "SELECT DISTINCT model FROM `duproduct` WHERE `manufacturer_id` = $id AND 
				product_id in (SELECT `product_id` FROM `duproduct_to_category` WHERE `category_id` = $path)";
		else
			$sql = "SELECT DISTINCT model FROM `duproduct` WHERE `manufacturer_id` = $id";
		$query = $this->db->query($sql);
		return $query;
	}
	return false;
...
// var_dump($data['filter_manufacturer_id']);
// var_dump($data['model']);
// exit;
	$query = $this->db->query($sql);
