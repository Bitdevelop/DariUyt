...
class ControllerModuleAnylist extends Controller {
	protected function index($setting) {
		...
		$this->data['shortdesc'] = $setting['shortdesc'][$languageId];
				$this->data['shortdesc2'] = $setting['shortdesc2'][$languageId];
		...
		if ((!$filter or $ok) and !$outofperiod) {
			...
			foreach ($results as $pid) {
				...
				if ($result) {
					if ($result['image']) {
						$image = $this->model_tool_image->resize($result['image'], 175, 135);
					} else {
						$image = false;
					}
